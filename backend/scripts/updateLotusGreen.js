...
const parseBuyerName = (value) => {
  if (!value) return null;
  
  const str = value.toString().trim();
  
  // Skip if it's "null", "N/A", empty, or just whitespace
  const lower = str.toLowerCase();
  if (lower === 'null' || lower === 'n/a' || lower === '' || lower === '-' || lower === 'na' || lower === 'undefined') {
    return null;
  }
  
  // Return the trimmed string (capitalize first letter of each word for consistency)
...
const findColumn = (row, possibleNames) => {
  ...
  const emptyMap = {
    ...
    '__EMPTY_7': ['Buyers name', 'Buyers Name', 'Buyer name', 'Buyer Name', 'ownerName', 'owner name']
  };
...
// 4. Main Execution
const updateLotusGreen = async () => {
  try {
    // Connect to MongoDB
    await mongoose.connect(process.env.DATABASE);
    console.log('✅ Connected to MongoDB');

    // Find Lotus Green project
    const project = await Project.findOne({ name: 'Lotus Green' });
    if (!project) {
      console.error('❌ ERROR: Project "Lotus Green" not found in database');
      process.exit(1);
    }
    
    console.log(`✅ Found project: Lotus Green (ID: ${project._id})`);
    ...
    const possibleFileNames = [
      'Lotus Green.xlsx',
      'All flats data of all sites.xlsx - Lotus Green.csv',
      'Lotus Green.csv'
    ];
    ...
    if (rows.length > 0) {
      ...
      // Check if buyer name column exists
      const allColumnNames = Object.keys(rows[0] || {});
      const buyerNameColumns = allColumnNames.filter(name => 
        name && (name.toLowerCase().includes('buyer') || name.toLowerCase().includes('owner') || name.toLowerCase().includes('name'))
      );
      console.log('\n🔍 Buyer/Owner name related columns found:');
      if (buyerNameColumns.length > 0) {
        buyerNameColumns.forEach(col => {
          console.log(`   - "${col}"`);
        });
      } else {
        console.log('   - No buyer/owner name columns found!');
      }
    ...
    console.log(`✅ Found ${allUnits.length} existing units for Lotus Green`);
    ...
    if (allUnits.length > 0) {
      ...
      allUnits.slice(0, 5).forEach(unit => {
        ...
        let buyerNameUpdateCount = 0;
        let buyerNameSkipCount = 0;

    for (const row of rows) {
      const unitNo = findColumn(row, ['unit No', 'unit No.', 'unit no', 'Unit No', 'Flat no.', 'Flat No', 'flat no', 'flat no.']);
      const wing = findColumn(row, ['wing', 'Wing', 'WING', 'towerOrWing']);
      const floorStr = findColumn(row, ['Floor', 'floor', 'FLOOR']);
      const carpetAreaStr = findColumn(row, ['Rera Carpet Area in (Sq.mtr)', 'RERA Carpet Area in (Sq.mtr)', 'Carpet Area', 'carpet area', 'Carpet area']);
      const buyerNameStr = findColumn(row, ['Buyers name', 'Buyers Name', 'Buyer name', 'Buyer Name', 'ownerName', 'owner name']);
      ...
      if (bulkOps.length === 0 && notFoundUnits.length < 3) {
        ...
        console.log(`   buyerNameStr: "${buyerNameStr}"`);
        const parsedBuyer = parseBuyerName(buyerNameStr);
        console.log(`   Parsed buyerName: ${parsedBuyer === null ? 'null (filtered out)' : `"${parsedBuyer}"`}`);
      }

      // Parse values
      const floor = parseFloor(floorStr);
      const carpetArea = parseCarpetArea(carpetAreaStr);
      const buyerName = parseBuyerName(buyerNameStr);
      ...
      if (buyerName !== null) {
        // Update buyer name (ownerName field)
        updateFields.ownerName = buyerName;
        buyerNameUpdateCount++;
      } else {
        // If buyerNameStr exists but was filtered out (e.g., "null"), or doesn't exist
        if (buyerNameStr !== null && buyerNameStr !== undefined) {
          buyerNameSkipCount++;
        }
      }
    ...
    // Execute bulk write
    if (bulkOps.length > 0) {
      console.log(`\n📊 Buyer name statistics:`);
      console.log(`   - Buyer names to update: ${buyerNameUpdateCount}`);
      console.log(`   - Buyer names skipped (null/empty): ${buyerNameSkipCount}`);
      ...
      if (result.modifiedCount > 0) {
        ...
        for (const unitId of sampleKeys) {
          ...
          if (updatedUnit) {
            ...
            console.log(`     - ownerName: ${updatedUnit.ownerName || 'NOT SET'}`);
          }
        }
        
        // Also verify a unit with buyer name if any were updated
        const unitWithBuyer = await Units.findOne({
          projectId: projectCode,
          ownerName: { $exists: true, $ne: null, $ne: '' },
          removed: { $ne: true }
        }).lean();
        
        if (unitWithBuyer) {
          console.log(`\n📋 Sample unit with buyer name:`);
          console.log(`   Unit ${unitWithBuyer.unitNumber} (Wing: ${unitWithBuyer.towerOrWing}):`);
          console.log(`     - ownerName: "${unitWithBuyer.ownerName}"`);
        }
      }
    } else {
