...
function unitsController() {
  ...
  methods.getUnitsByProjectCode = async (req, res) => {
    try {
      ...
      const project = await Project.findOne({ projectCode, removed: { $ne: true } });
      if (!project) {
        return res.status(404).json({
          success: false,
          result: null,
          message: `Project not found with projectCode: ${projectCode}`,
        });
      }

      // ❗ Units store projectCode in projectId field (string)
      ...
      return res.status(200).json({
        success: true,
        result: units,
        message: `Units found for projectCode ${projectCode}`,
      });
