...
export default function PendingReport() {
    ...
    const fetchData = async (currentFilters) => {
        ...
        try {
            ...
            const res = await request.get({ entity: 'report/pending', query });
            if (res.success) {
                setData(res.result || []);
            }
        } finally {
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(wb, `Pending_Report_${dayjs().format('YYYY-MM-DD')}.xlsx`);
    };
    ...
    const exportToPDF = () => {
        ...
        doc.setFontSize(16);
        doc.text("Pending Activities Report", 14, 15);
        doc.setFontSize(10);
        ...
        doc.save(`Pending_Report_${dayjs().format('YYYY-MM-DD')}.pdf`);
    };
        ...
        <Layout style={{ minHeight: '100vh', background: '#fafafa' }}>
            <Content style={{ padding: '32px 24px' }}>
                <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <Title level={2}>Pending Activities Report</Title>
                        <Text type="secondary">View and export activities not completed in selected time</Text>
                    </div>
                    <Space>
