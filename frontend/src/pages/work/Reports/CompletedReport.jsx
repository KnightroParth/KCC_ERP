...
export default function CompletedReport() {
    ...
    const fetchData = async (currentFilters) => {
        ...
        try {
            ...
            const res = await request.get({ entity: 'report/completed', query });
            if (res.success) {
                setData(res.result || []);
            }
        } finally {
    ...
    const columns = [
        ...
        { title: 'Task', dataIndex: 'activityName', key: 'activityName' },
        {
            title: 'Contractor',
        ...
        },
        {
            ...
            dataIndex: 'unitId',
            key: 'building',
            render: (u) => u?.buildingName || u?.towerOrWing || '-'
        },
        {
            title: 'Unit/Flat No.',
            dataIndex: 'unitId',
    ...
    const getProcessedData = () => {
        ...
        categories.forEach(cat => {
            ...
            const subTotal = catItems.reduce((sum, item) => sum + (item.defaultRate || 0), 0);
            grandTotal += subTotal;

            processed.push(...catItems.map((item) => ({ ...item, key: item._id })));

            processed.push({
                key: `subtotal-${cat}`,
                category: `SUB TOTAL (${cat})`,
                isSubTotal: true,
                defaultRate: subTotal
            });
        });
    ...
    const exportToExcel = () => {
        ...
        const excelData = processedData.map(row => ({
            ...
            'Task': row.activityName || '',
            'Contractor': row.contractorId?.companyName || row.contractorId?.name || '',
            'Building': row.unitId?.buildingName || row.unitId?.towerOrWing || '',
            'Unit/Flat No.': row.unitId?.unitNumber || '',
            'Rate': row.defaultRate
        }));
        const ws = XLSX.utils.json_to_sheet(excelData);
        XLSX.utils.book_append_sheet(wb, ws, "Completed Report");
        XLSX.writeFile(wb, `Completed_Report_${dayjs().format('YYYY-MM-DD')}.xlsx`);
    };
    ...
    const exportToPDF = () => {
        ...
        doc.setFontSize(16);
        doc.text("Completed Activities Report", 14, 15);
        doc.setFontSize(10);
        ...
        const tableRows = processedData.map(row => [
            ...
            row.unitId?.buildingName || row.unitId?.towerOrWing || '',
        ...
        doc.save(`Completed_Report_${dayjs().format('YYYY-MM-DD')}.pdf`);
    };
        ...
        <Layout style={{ minHeight: '100vh', background: '#fafafa' }}>
            <Content style={{ padding: '32px 24px' }}>
                <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <Title level={2}>Completed Activities Report</Title>
                        <Text type="secondary">View and export activities completed in selected time</Text>
                    </div>
                    <Space>
